...
async function getAppCallTxn (id: number, from: algosdk.Account, receiver: algosdk.Account, amount: number) {
...
describe.only('App Call', () => {
  let funder: algosdk.Account
  let creator: algosdk.Account
  let receiver: algosdk.Account
  let appAccount: algosdk.Account
  let preReceiverBalance: number
  let preAppBalance: number

  const itxnAmount = 1_001
  ...
  beforeAll(async () => {
    ...
    appAccount = { addr: algosdk.getApplicationAddress(appID), sk: new Uint8Array() } as algosdk.Account

    await fundAccount(funder, appAccount, 1_000_000)

    preReceiverBalance = (await algodClient.accountInformation(receiver.addr).do()).amount
    preAppBalance = (await algodClient.accountInformation(appAccount.addr).do()).amount

    const appCallTxn = await getAppCallTxn(appID, creator, receiver, itxnAmount)
    await createDryRunFromTxns([appCallTxn], 'app_call')

    await sendTxn(appCallTxn)
  })

  it('receiever balance increase', async () => {
    const balance = (await algodClient.accountInformation(receiver.addr).do()).amount
    expect(balance).toBe(preReceiverBalance + itxnAmount - 1_000)
  })

  it('app balance decrease', async () => {
    const balance = (await algodClient.accountInformation(appAccount.addr).do()).amount
    expect(balance).toBe(preAppBalance - itxnAmount)
  })
